...
class TestWebServerEndpoints:
    ...
    def test_audio_provider_discovery_reads_live_named_instances(self, monkeypatch):
        ...
        monkeypatch.setattr(
            tts_registry,
            "list_providers",
            lambda: [SimpleNamespace(name="plugin-tts", display_name="Plugin TTS")],
        )
        ...
        tts = {entry["name"]: entry for entry in body["tts"]["provider_details"]}
        stt = {entry["name"]: entry for entry in body["stt"]["provider_details"]}

        assert body["tts"]["current"] == "supertonic-m1"
        assert tts["supertonic-m1"] == {
            "name": "supertonic-m1",
            "label": "Supertonic M1",
            "family": "supertonic",
            "type": "command",
        ...
        assert stt["parakeet-lan"]["label"] == "Parakeet LAN"
        assert tts["plugin-tts"]["type"] == "plugin"
